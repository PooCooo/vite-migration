...
mock-vite-migration/
  ...
  pages-php/                     # 真 PHP 模板：与 pages/ 一一对应，CSS link 由 PHP 实时注入
    home.php
    result.php
  lib/manifest.php               # PHP 端 manifest 读取：CSS link 渲染 + 业务 entry URL 查表 (方案已废弃,只在DEV HMR下使用,后续会完全删除)
...
- 阶段三：在当前项目跑通stc逻辑；兼容stc的相关限制见 `MIGRATION_COMPATIBILITY_CHECKLIST.md`。
...
- 已验证命令：`npm run test:run`、`npm run build:vite`、`npm run check:stc`、`php -l pages-php/home.php`、`php -l pages-php/result.php`、`php -l lib/manifest.php`。
...
- `scripts/check-stc-build.mjs` 是阶段 2 后续改动的守卫：检查稳定产物清单、无 vendor/manifest、JS 无 inline CSS / 动态 import、模板保留 `/resource/...` STC 字面量且不回退到 `dist-vite` / manifest helper / `{ stc, legacy }`。
...
- 修改 Vite 生产构建、模板资源引用或 CSS 输出后，至少运行 `npm run build:vite && npm run check:stc`。
