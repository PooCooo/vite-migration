...
- 迁移兼容性清单已整理到 `MIGRATION_COMPATIBILITY_CHECKLIST.md`
...
已整理到 `MIGRATION_COMPATIBILITY_CHECKLIST.md`，作为阶段三落地输入。清单覆盖：

- 推荐架构：dev shim、prod manifest + loader 查表、全局库分层加载
- Vite 配置要点：多入口、plugin-vue、plugin-legacy、Vue 进依赖图、生产恢复 hash
- Manifest 方案：PHP 注入统一 manifest，业务 CSS 由模板层 `<link>` 渲染
- `_loader_res.js` 改造清单：业务/全局库分流、modern/legacy 分支、`toAbsoluteUrl`
- PHP 模板最小改动、全局库策略、CSS 策略、legacy/polyfill 注意事项
- 阶段三启动前必须确认的问题和不应从 mock 照搬的内容
